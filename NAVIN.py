...
admin_users = {1866961136}  # Replace with actual admin user IDs
...
async def disapprove(update: Update, context: CallbackContext):
    ...
    async def admin_disapprove(update: Update, context: CallbackContext):
    user_id = update.effective_user.id
    if user_id not in admin_users:
    ...
    if len(context.args) != 1:
        await update.message.reply_text("Usage: /admin_disapprove <user_id>")
        return

    user_to_disapprove = int(context.args[0])
    approved_users.discard(user_to_disapprove)
    await update.message.reply_text(f"Admin disapproved user {user_to_disapprove}.")
...
def main():
    ...
    application.add_handler(CommandHandler("admin_disapprove", admin_approve)) #TO REMOVE ADMIN 
